...
void IcebergMultiFileList::ScanPositionalDeleteFile(DataChunk &result, const string &delete_file_name) const {
	...
	if (it == positional_delete_data.end()) {
		it =
		    positional_delete_data.emplace(initial_key, make_uniq<IcebergPositionalDeleteData>(delete_file_name)).first;
	}
	...
	for (idx_t i = 0; i < count; i++) {
		...
		if (name != current_file_path.get()) {
			...
			if (it == positional_delete_data.end()) {
				it =
				    positional_delete_data.emplace(key, make_uniq<IcebergPositionalDeleteData>(delete_file_name)).first;
			}
