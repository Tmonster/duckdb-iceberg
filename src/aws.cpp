...
unique_ptr<HTTPResponse> AWSInput::ExecuteRequest(ClientContext &context, RequestType request_type,
	...
	// 	return uri;
	// }
	auto uri = IRCEndpointBuilder();
	uri.SetHost(authority);
	for (auto &segment : path_segments) {
		uri.AddPathComponent(segment);
	}
	for (auto &parameter: query_string_parameters) {
		uri.SetParam(parameter.first, parameter.second);
	}
	...
	string url_encoded_path = uri.GetURLEncoded();
	...
	auto canonical_request =
	    string(EnumUtil::ToString(request_type) + "\n" + url_encoded_path + "\n");
	if (uri.GetURLEncoded().size()) {
		canonical_request += uri.GetURLEncoded().substr(1);
	}
	...
	string request_url = uri.GetURLEncoded();
...
unique_ptr<HTTPResponse> AWSInput::Request(RequestType request_type, ClientContext &context,
	...
	switch (request_type) {
	case RequestType::GET_REQUEST:
		return ExecuteRequest(context, request_type, client, headers);
	case RequestType::POST_REQUEST:
		return ExecuteRequest(context, request_type, client, headers, data);
	case RequestType::DELETE_REQUEST:
		return ExecuteRequest(context, request_type, client, headers);
	case RequestType::HEAD_REQUEST:
		return ExecuteRequest(context, request_type, client, headers);
