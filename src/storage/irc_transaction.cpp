...
void CommitTableToJSON(yyjson_mut_doc *doc, yyjson_mut_val *root_object,
	...
	for (auto &update : table.updates) {
		if (update.has_add_snapshot_update) {
			...
			for (auto &additional_propery : snapshot.summary.additional_properties) {
				yyjson_mut_obj_add_strcpy(doc, summary_json, additional_propery.first.c_str(),
				                          additional_propery.second.c_str());
			}
