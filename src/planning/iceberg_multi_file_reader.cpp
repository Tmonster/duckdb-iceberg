...
void IcebergMultiFileReader::ApplyEqualityDeletes(ClientContext &context, DataChunk &input_chunk,
	...
	//! Apply equality deletes. The BoundReferenceExpression indexes inside
	//! `equality_delete_filter` are physical positions in `input_chunk` (which is
	//! the file reader's local chunk: it contains the non-virtual entries of
	//! `column_indexes` in their original order). Run the predicate against
	//! `input_chunk` and slice `output_chunk` with the resulting selection vector:
	//! both chunks have identical cardinality and parallel row order.
	...
	output_chunk.SetCardinality(count);
}
