...
void IcebergMultiFileList::ScanDeleteFiles(const vector<MultiFileColumnDefinition> &global_columns,
	...
	for (; shared_state->next_delete_entry_to_process < shared_state->delete_manifest_entries.size();
		...
		//! Skip delete files whose partition (or equality-value bounds) is excluded by the scan filter: they
		//! cannot delete a row from any surviving data file, so reading and materializing them is wasted work.
