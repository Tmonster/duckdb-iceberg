...
void IcebergMultiFileList::EnumerateDeleteManifestEntries() const {
	// In <=v2 we have to process *all* delete manifests before we can be certain we have all the
	// delete data for the current file.
	//
	// v3 solves this — `referenced_data_file` tells us which file the data_file is targeting before
	// we open it, and there can only be one deletion vector per data file. Spec: "At most one
	// deletion vector is allowed per data file in a snapshot."
...
void IcebergMultiFileList::ProcessDeletes(const vector<MultiFileColumnDefinition> &global_columns,
	...
	while (delete_scan_watermark < delete_manifest_entries.size()) {
